...
phase_send() {
  local windows inventory meta_pane foreign_pane
  : > "$LOG"
  printf '❯\n' > "$PANE"
  # The meta window (firstmate:fm-design) must win over a foreign same-named
  # window in another session. Include the recorded endpoint in the fake
  # inventory so the recovery-grade liveness check can verify it exists.
  windows="firstmate:fm-design
other-session:fm-design"
  PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_TMUX_WINDOW="$windows" \
    FM_FAKE_TMUX_LOG="$LOG" FM_FAKE_TMUX_CAPTURE="$PANE" \
  ...
  # Keys go to the exactly resolved pane id, so read which pane the fake's
  # inventory holds for each same-named window.
  inventory=$(PATH="$FAKEBIN:$PATH" FM_HOME="$HOME_DIR" FM_FAKE_TMUX_WINDOW="$windows" \
    FM_FAKE_TMUX_LOG=/dev/null tmux list-panes -a -F inventory)
  meta_pane=$(printf '%s\n' "$inventory" | awk -F: '$5 == "firstmate" && $6 == "fm-design" { print $3 }')
  foreign_pane=$(printf '%s\n' "$inventory" | awk -F: '$5 == "other-session" && $6 == "fm-design" { print $3 }')
  [ -n "$meta_pane" ] && [ -n "$foreign_pane" ] && [ "$meta_pane" != "$foreign_pane" ] \
    || fail "the fake inventory does not hold both same-named windows as distinct panes"$'\n'"$inventory"
  ...
  assert_grep "send-keys -t $meta_pane -l : Firstmate instruction waiting:" "$LOG" "send did not ring the window recorded in this home's meta"
  assert_no_grep 'route this work' "$LOG" "send typed the payload instead of only the doorbell"
  assert_no_grep "send-keys -t $foreign_pane " "$LOG" "send targeted a foreign same-named window"
