...
4. Escalate only genuinely ambiguous findings:
   ...
   A standing answer the captain has already given for a class of finding in the current task satisfies that captain ownership for the rest of that task, so applying it to a later finding of the same class relays the captain's decision rather than answering in their place, under the `Captain-facing escalation` contract below.
...
This batches the captain's decision rather than replacing it: a genuinely different question is still its own escalation, a finding outside the class the captain answered, or inside step 4's questionable-abstraction or stronger-boundary conditions, still escalates on its own, and the standing answer expires with that task so follow-up work never inherits it.
